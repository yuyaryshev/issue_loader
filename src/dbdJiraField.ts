...
export const dbdDJiraFieldInput = {
    name: "JIRA_FIELD",
    table: "JIRA_FIELD",
    changesTable: "JIRA_FIELD_CHANGES",
    handlerStoredProc: "HANDLE_JIRA_FIELD_CHANGES",
    fields: [
        ...
        { name: "TARGET_NAME", type: "string40", nullable: true, pk: false, insert: false } as DbDomFieldInput,
        ...
        { name: "LOAD_ALG_OVERRIDE", type: "string40", nullable: true, pk: false, insert: false },
        { name: "ISSUE_LOADER_TYPE_OVERRIDE", type: "string40", nullable: true, pk: false, insert: false },
        { name: "ORACLE_TYPE_OVERRIDE", type: "string40", nullable: true, pk: false, insert: false }
    ] as DbDomFieldInput[]
};
    ...
    switch (f.TYPE) {
        ...
        default:
            if (!unknownJiraTypes.has(f.TYPE)) {
                ...
                yconsole.warn(`T1008`, `Unknown fieldMarked.TYPE = '${f.TYPE}', mapped to 'varchar2(255)'`);
            }
            return "string4000";
    }
};
...
export const readDJiraFieldMarkedMeta = async function(
    ...
    if (!result.rows || !result.rows.length) {
        ...
        if (!result.rows || !result.rows.length)
            throw new Error(
                `T1200 Oracle haven't returned result for "${sql}" - Jira is unreachable or some internal error.`
            );
    }
    ...
    if (!markedFields.length)
        throw new Error(`T1201 No marked fields exist inside '${tableName}' - aborting all operaions.`);
    return markedFields;
};
